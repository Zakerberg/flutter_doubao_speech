...
class DoubaoSpeechPlugin : FlutterPlugin, MethodCallHandler, StreamHandler {
    ...
    private fun initEngine(call: MethodCall, result: Result) {
        ...
        args?.get("logPath")?.let {
            engine.setOptionString(SpeechEngineDefines.PARAMS_KEY_DEBUG_PATH_STRING, it as String)
        }
        args?.get("logLevel")?.let {
            engine.setOptionString(SpeechEngineDefines.PARAMS_KEY_LOG_LEVEL_STRING, it as String)
        }
        ...
        // 播放器配置
        engine.setOptionBoolean(SpeechEngineDefines.PARAMS_KEY_TTS_ENABLE_PLAYER_BOOL, true)
        engine.setOptionBoolean(
                SpeechEngineDefines.PARAMS_KEY_ENABLE_PLAYER_AUDIO_CALLBACK_BOOL,
                true
        )
        ...
        args?.get("workMode")?.let {
            engine.setOptionInt(SpeechEngineDefines.PARAMS_KEY_DIALOG_WORK_MODE_INT, it as Int)
        }
        ...
        args?.get("customSampleRate")?.let {
            engine.setOptionInt(SpeechEngineDefines.PARAMS_KEY_CUSTOM_SAMPLE_RATE_INT, it as Int)
        }
        args?.get("customChannel")?.let {
            engine.setOptionInt(SpeechEngineDefines.PARAMS_KEY_CUSTOM_CHANNEL_INT, it as Int)
        }
    ...
    private fun handleMessage(type: Int, data: ByteArray?) {
        when (type) {
            ...
            SpeechEngineDefines.MESSAGE_TYPE_EVENT_ASR_INFO -> sendEvent("asr_start", null)
    ...
    private fun sendAudioEvent(type: String, data: ByteArray) {
        eventSink?.let { sink ->
            val event =
                    mapOf("type" to type, "audio" to Base64.encodeToString(data, Base64.NO_WRAP))
            sink.success(event)
        }
    }
